...
void GetSwitchAction(RC_Ctl_t g_RcRawData){
	...
	if( S1LastSta==0)
	{
	S1LastSta = g_RcRawData.rc.s1;
	S2LastSta = g_RcRawData.rc.s2;
	}
		 
	CC_Condition.S1Sta = (SwitchState)g_RcRawData.rc.s1;
	CC_Condition.S2Sta = (SwitchState)g_RcRawData.rc.s2;

	if( g_RcRawData.rc.s1 != S1LastSta )
	{
		switch( g_RcRawData.rc.s1 )
		{
			case 1:
	...
	if( g_RcRawData.rc.s2 != S2LastSta )
	{
		switch( g_RcRawData.rc.s2 )
		{
			case 1:
...
void RemoteTaskLoop( RC_Ctl_t g_RcRawData )
{
