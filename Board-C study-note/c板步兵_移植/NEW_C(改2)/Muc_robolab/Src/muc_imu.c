...
volatile float IMUAngleData = 0;
volatile IMU_REAL_DATA  IMU_Real_Data;
...
void get_angle(fp32 q[4], fp32 *yaw, fp32 *pitch, fp32 *roll)
{
	...
	*roll = atan2f(2.0f*(q[0]*q[1]+q[2]*q[3]),2.0f*(q[0]*q[0]+q[3]*q[3])-1.0f);
	...
	IMU_Real_Data.Gyro_Z = *pitch;
	IMU_Real_Data.Gyro_Y = *yaw;
	IMU_Real_Data.Gyro_X = *roll;
}
...
void MucImuInit()
{
		/* 官方代码使用的是osDelay() ,虽然延时时间一样，但实现方式不同，HAL_Delay()会一直占用CPU，应该影响不大*/
		HAL_Delay(7);  
    while(BMI088_init())
    {
       HAL_Delay(100);
...
void ImuAnalysisTask()
{
		static uint8_t firstRun = 1;
	
/* 这个函数的作用是接受line_0外部中断里的任务通知，既然程序框架已经改变，那它现在已经不需要了。
如果这个思路是错的，可以使用一个FLAG变量实现	*/
//		//等待SPI DMA传输
//		while (ulTaskNotifyTake(pdTRUE, portMAX_DELAY) != pdPASS){}
		...
		if( firstRun )
		{
				IMUAngleData = GMYawEncoder.ecd_angle;
				firstRun = 0;
		}
		IMUAngleData -= IMU_Real_Data.Gyro_Z*0.001f;
}