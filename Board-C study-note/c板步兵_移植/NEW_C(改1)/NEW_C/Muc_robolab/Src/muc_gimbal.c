...
//    VAL_LIMIT( GimbalRC->yaw_angle_ref,-60,60 );
    ...
    if( Sta_To_Circle ==1)
	{
	   VAL_LIMIT( CC_Condition.pitch_angle_ref, -25, 30 );
	}
	else
	{
	   VAL_LIMIT( GimbalRC->pitch_angle_ref, -25, 30 );			//抬头低头
    ...
    /* 设置云台电机的速度反馈信息 */
    pitch_angle_vel_ecd = IMU_Real_Data.Gyro_X;    //云台PITCH速度
    yaw_angle_vel_ecd = -IMU_Real_Data.Gyro_Z;     //云台YAW速度
...
//    PID_Calc( &Pitch_current_pid,pitch_angle_cur_ecd, Pitch_speed_pid.output_compensation);
	...
	PID_Calc( &Yaw_speed_pid,yaw_angle_vel_ecd,Yaw_position_pid.output_compensation);
	//PID_Calc( &Yaw_speed_pid,0,5);
	//own
	
	//PID_Calc( &Yaw_speed_pid, yaw_angle_vel_ecd,Yaw_position_pid.output);
	
	
	Yaw_current_pid.Kp=3;//110
	Yaw_current_pid.Ki=0.005;//60
	Yaw_current_pid.Kd=0;
	PID_Calc( &Yaw_current_pid, yaw_angle_cur_ecd,Yaw_speed_pid.output_compensation);	
   

	//GimbalDataSend(GimbalSta,0,0);
	GimbalDataSend(GimbalSta,Yaw_current_pid.output,Pitch_speed_pid.output);
	//GimbalDataSend(GimbalSta,Pitch_speed_pid.output,Yaw_speed_pid.output);
